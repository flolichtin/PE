"
* same as apollo.13 but excluding non-sig education level
* apollo.12 is better
"
...
apollo_fixed <- ref_level(apollo_fixed, ref = "educ_secondary")
apollo_fixed <- ref_level(apollo_fixed, ref = "educ_higher")
...
# non-sig education level
at <- apollo_tex(apollo.13)
...
fix <-
  dat.coef %>%
  filter(str_detect(coef, "mandatory"), pval >= 0.05) %>%
  pull(coef)
...
apollo.14 <- apollo_estimate(apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs)
texreg::screenreg(apollo_tex(apollo.14))

usethis::use_data(apollo.14, overwrite = TRUE)
