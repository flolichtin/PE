...
* CO2 initial level (only for undercomp)
"
...
# add co2 certificate price
price_certificate. <-
  database. %>%
  select(ID, price_certificate)

database <-
  database %>% left_join(price_certificate., by = "ID")
...
# rescale
database$co2_initial <- database$co2_initial / 1000
...
apollo_probabilities <- function(apollo_beta, apollo_inputs, functionality = "estimate")
{
  ...
  V[["crtfct"]] = asc_crtfct +
    ...
    b_crtfct_price * price_certificate
...
apollo.4 <- apollo_estimate(apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs)
texreg::screenreg(pemdcev::apollo_tex(apollo.4))

usethis::use_data(apollo.4, overwrite = TRUE)
